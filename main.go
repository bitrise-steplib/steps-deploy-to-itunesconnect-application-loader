...
func getAuthOptions(cfg Config) ([]string, string, error) {
	...
	if passErr != nil {
		...
		if apiErr != nil {
			return []string{}, "", fmt.Errorf("neither usen and password or API key and issuer ID is provided correctly, please define one of the pairs in the config.\nIssue with user and password: %s\nIssue with API key and issuer ID: %s", passErr, apiErr)
		}
...
func getAPIKeyAndIssuerOptions(cfg Config) ([]string, error) {
	...
	return []string{"--apiKey", cfg.APIKey, "--apiIssuer", cfg.APIIssuer}, nil
}
